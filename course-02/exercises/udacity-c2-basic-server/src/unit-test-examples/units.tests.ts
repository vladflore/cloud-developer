...
describe('concat', () => {
  ...
  it('should concatenate two non-empty string', () => {
    const result = concat('vlad ', 'flore');
    expect(result).to.equal('vlad flore');
  });

  it('should throw error when one of the strings is empty', () => {
    expect(() => concat('vlad', '')).to.throw('one of the input strings is empty');
  });
